...
const RAW_API_KEY = Constants.expoConfig?.extra?.HACKCLUB_API_KEY || process.env.HACKCLUB_API_KEY;
const API_KEY = RAW_API_KEY && !RAW_API_KEY.includes('YOUR_') && !RAW_API_KEY.includes('HERE') && !RAW_API_KEY.includes('your_')
  ? RAW_API_KEY.replace(/["']/g, "").trim() 
  : "";
...
// Use Expo proxy in development to avoid CORS issues
...
export const generateStoryOutline = async (
    ...
    const prompt = `
    You are a storyteller, but more of a like tourist tourist guide who explains diffrent places and their importance, make it historical, who can create a story outline for a journey that is${totalSegments} chapters long. 
